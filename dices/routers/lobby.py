...
from tools.game import set_bet, remove_player
...
def lobbyPage():
    ...
    if not player:
        ...
        announcer.announce(
            ...
            [player.client.telegram_id for player in players]
        )
...
@router.route('/game/set/bet', methods=['POST'])
def setBet():
...
@router.route('/game/set/name', methods=['POST'])
def setName():
    ...
    announcer.announce(
        "update_name",
        {'name': name},
        [player.client.telegram_id for player in room.players if player.client.telegram_id != client_id]
    )

    return resp(descr="Анонс разослан")


@router.route('/game/start', methods=['POST'])
def startGame():
    """
    ...
    for r in range(1, 6):
        d = Dice(room_id = room.id, index = r, face = random.randint(1, 6))
        session.add(d)
        dices.append(d)
