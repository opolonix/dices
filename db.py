...
class DiceModel(db.Model):
    ...
    glb: str = Column(String(200))
    avatar: str = Column(String(200))
    collection: str = Column(String(200))
    auto_update: bool = Column(Boolean, default=False, comment="По умолчанию заголовок и описание кости автоматически подтягиваются из нфт коллекции")

class ClientDice(db.Model):
    __tablename__ = 'client_dice'

    id: int = Column(Integer, primary_key=True, autoincrement=True)
    index: int = Column(Integer)
    model_id: int = Column(Integer, ForeignKey(DiceModel.id))
    client_id: int = Column(Integer, ForeignKey(Client.id))
    model: Mapped[DiceModel] = relationship()
...
class Player(db.Model):
    ...
    barrels: int = Column(Integer, default = 0)
...
class Step(db.Model):
    ...
    stage: int = Column(Integer, default = 1) # 1 - бросок разослан; 2 - бросок подтвержен; 3 - ход является завершенным
...
class Dice(db.Model): # к каждой комнате привыязано по пять кубиков
    ...
    id: int = Column(Integer, primary_key=True, autoincrement=True)
    room_id: int = Column(Integer, ForeignKey(Room.id))
    step_id: int = Column(Integer, ForeignKey(Step.id))
    index: int = Column(Integer)
...
def as_dict(*slas) -> dict:
    result = {}
    for sla in slas:
        if sla.__table__.name == "clients": names = ['first_name', 'avatar']
        else: names = [c.name for c in sla.__table__.columns]
        for c in names:
            data = getattr(sla, c)
        ...
        if sla.__table__.name == "dices": # для таблицы дайсов заменяем id на index и убираем индекс
